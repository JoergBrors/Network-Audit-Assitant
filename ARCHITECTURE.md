...
1. Nutzer klickt „KI-Analyse“ (nur aktiv, wenn ein Modell geladen ist) und gibt einen **Sanitizer-Schlüssel** ein (frei wählbar, nur für diese Sitzung im UI-State, nicht in `sessionStorage`/`localStorage`, nicht geloggt).
...
- **Sitzungsverzeichnis** (`src/ui/workspace/aiSessionDirectory.ts`): Eine kleine, rein browserlokale (`localStorage`, Schlüssel `ai-session-directory`) Historie vergangener/aktueller Sitzungen – **nur Metadaten** (Zeitpunkt, Titel = erste Modellantwort gekürzt auf 140 Zeichen, Anzahl Nachrichten, Status `active`/`ended`). Es werden nie Chatinhalte, der Export oder der Sanitizer-Schlüssel gespeichert, da diese Daten ohnehin mit dem Sitzungsende aus Azure OpenAI gelöscht werden (§ 22.1) und keinen Grund haben, dauerhaft im Browser zu verbleiben. Beendete Sitzungen sind nur als Historie sichtbar, nicht reaktivierbar (ihr Vector Store existiert nicht mehr).
