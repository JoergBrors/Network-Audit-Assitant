...
**Delegierte Subnets:** Den Nutzer eines delegierten Subnets trägt die Plattform am Subnet selbst ein (`serviceAssociationLinks` / `resourceNavigationLinks` mit `linkedResourceType` und `link`, Teil der VNet-Zeile von Q-NET-VNET). Die Normalisierung übernimmt sie als `subnets[].serviceLinks` und ordnet das Subnet dem genannten Dienst zu – so wird z. B. das Integrations-Subnet eines App Service Plans (`Microsoft.Web/serverFarms`) gefunden, und Apps mit klassischer VNet-Integration (nur `vnetName` in `config/web`) erben es vom Plan. Neu im Katalog dafür: App Service Plan (`microsoft.web/serverfarms`), Application Gateway for Containers (`microsoft.servicenetworking/trafficcontrollers` mit den Kindressourcen `associations`/`frontends`) und Managed DevOps Pools. AKS Virtual Nodes werden über `addonProfiles.aciConnectorLinux.config.SubnetName` zugeordnet. Steht im Link eine Ressource, die nicht im Inventar ist, meldet die Bewertung DELEGATION_USER_NOT_DISCOVERED (andere Subscription oder fehlende Leserechte); Delegationen ganz ohne Nutzer DELEGATION_UNUSED.

**Fabric / Power BI:** Ohne Private-Link-Dienst für Fabric/Power BI im Inventar gilt eine Kapazität als öffentlich erreichbar (Microsoft-Standard, Schutz nur über Entra ID) – Befund FABRIC_NO_PRIVATE_LINK. Mit Private Link bleibt die Erreichbarkeit „unbekannt“, weil „Block Public Internet Access“ nur im Admin-Portal steht.
